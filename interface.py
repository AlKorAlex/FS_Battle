...
class Interface():
    ...
    def page(self, page, player_turn = 2):
        ...
        match page:
            ...
            case 'combat_interface':
                self.combat_interface(player_turn)
    ...
    def combat_interface(self, player_turn):
        ...
        for i in range(0, len(self.game_stats.hand_cards[0])): # Рисует карты на руке атакующего
            self.c_card = self.game_stats.hand_cards[0][i]
            if player_turn == 0: # Сторона карты
                self.card_turn = 'up'
            else:
                self.card_turn = 'back'
            ...
            if i < 3: # Рисует первые 3 карты на руке атакующего
                ...
                self.c_card.draw_card(self.screen, played_cards_position_x, played_cards_position_y, self.card_turn)
        ...
        for i in range(0, len(self.game_stats.using_battle_cards[0])):  # Рисует сыгранные карты атакующего
            self.c_card = self.game_stats.using_battle_cards[0][i]
            played_cards_position_x = (i + 1) * w_s + i * self.c_card.surf.get_width()
            played_cards_position_y = w_s * 2
            self.c_card.draw_card(self.screen, played_cards_position_x, played_cards_position_y, 'up')


        for i in range(0, len(self.game_stats.hand_cards[1])):
            self.c_card = self.game_stats.hand_cards[1][i]
            if player_turn == 1: # Сторона карты
                self.card_turn = 'up'
            else:
                self.card_turn = 'back'
        ...
        for i in range(0, len(self.game_stats.using_battle_cards[1])):  # Рисует сыгранные карты защищающегося
            self.c_card = self.game_stats.using_battle_cards[1][i]
            played_cards_position_x = 2.1 * self.settings.screen_width // 3.2 + (
                        i + 1) * w_s + i * self.c_card.surf.get_width()
        ...
        for j in range(0, 2): # Отрисовывает кубики игроков
            for i in range(0, len(self.game_stats.all_dice[j])):  # Отрисовывает кубики

                dice = self.game_stats.all_dice[j][i]
                x = self.settings.screen_width * 1.1 // 3.2 * abs(j-1) + j * self.screen.get_rect().midtop[0] # Начальное положение (Стенка)
                w_s = (self.settings.screen_width // 6.4 - 4 * dice.surf.get_width()) // 5
                if i > 3:
                    played_cards_position_x = x + w_s * (i - 3) + dice.surf.get_width() * (i - 4)
                    played_cards_position_y = 2 * w_s + dice.surf.get_height()
                    dice.draw(self.screen, played_cards_position_x, played_cards_position_y)

                else:
                    played_cards_position_x = x + w_s * (i + 1) + dice.surf.get_width() * i
        ...
        for j in range(0, 2): # Отрисовывает юнитов игроков
            k = 0
            for i in range(0, len(self.game_stats.army[j])):
                unit = self.game_stats.army[j][i] # Конкретный юнит
                ...
                played_unit_position_x = x + w_s * (k + 1) + unit.surf.get_width() * k
                if k == 0:
                    k = 1
                else:
                    k = 0
                played_unit_position_y = self.settings.screen_height // 3 + w_s * int(i/2) + int(i/2) * unit.surf.get_height() + w_s
...
class Dice_Window(): # Окно с выбором кубика
    def __init__(self, screen, game_stats, player_number):
        ...
        self.sc_text = f.render('Выберите кубик', 1, Settings().RED, Settings().BLACK)  # Надпись действия
        self.pos = self.sc_text.get_rect(center=(500 // 2, 500 // 2))
